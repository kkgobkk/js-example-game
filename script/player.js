...
let update_player_position = function(){
    if(player.recoil_time > 0){
        player.recoil_time --;
        if(!(player.pressing_right || player.pressing_left))
            player.x -= player.recoil_x * player.recoil_time / 10;
        if(!(player.pressing_down || player.pressing_up))
            player.y -= player.recoil_y * player.recoil_time / 10;
    }
